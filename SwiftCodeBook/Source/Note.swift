...
2) 对于某个VC，对其调用dismiss方法时：
 如果该VC有present的下级VC，则会dismiss掉该VC所有present的下级VC，注意自身不会被dismiss
 如果该VC没有present的下级VC，且该VC自身也是被present出来的，则会dismiss掉该VC自己
 如果该VC没有present的下级VC，且该VC自身不是被present出来的，则dismiss无效但completion仍会被调用
