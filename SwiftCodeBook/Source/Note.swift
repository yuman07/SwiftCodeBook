...
/*
 ...
 在iOS中有三种实现线程安全的方式：actor，queue，lock
 实现一个线程安全的类必须同时保证：
 1) 串行调用时，外界调用顺序与实际执行顺序相同
 2) 并行调用时，没有数据竞争问题
 对于
 3) 并行调用时，外界调用顺序与实际执行顺序相同
 这点主要是在unfair lock下不满足，但绝大多数情况下不满足也不会有问题
 因为在多线程下调用方法，实际执行的顺序是不确定的。比如按顺序123往并发队列中添加方法，但实际的执行顺序可能是213
