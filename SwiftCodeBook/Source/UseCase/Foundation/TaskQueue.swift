...
// 即此时该Task内的block均运行于主线程(当然因为是主线程肯定也是线性的)

// 如果当前上下文没有Actor信息，比如就在一个普通的class中使用Task{...}
// 那该Task的block会运行在一个并发队列(其实在老的iOS版本中是一个串行线程，但我们不应该去假定其背后的实现，即我们不能保证该Queue一定是串行的)
// 那这样会造成可能的多线程问题，比如我们需要流式计算一个网络响应数据的hash值，尽管回调是线性的，但由于Task的block是并行的，因此可能先计算后面的data再计算前面的
// 即有些时候我们确实需要一个Task内的block是线性执行
